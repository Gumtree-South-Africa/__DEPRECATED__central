...
public class MessageProcessingCoordinator {
    ...
    private void handleTermination(Termination termination, String messageId, Optional<Mail> mail, Optional<DefaultMutableConversation> conversation, byte[] messageBytes) {
        checkNotNull(termination);
        checkNotNull(messageId);
        checkNotNull(messageBytes);

        DefaultMutableConversation c = conversation.isPresent() ? conversation.get() :
