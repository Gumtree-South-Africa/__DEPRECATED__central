...
public class PostBoxInitializer {
    ...
    @Autowired
    public PostBoxInitializer(PostBoxRepository postBoxRepository, @Value("${replyts.maxPreviewMessageCharacters:250}") int maxChars) {
    ...
    public void moveConversationToPostBox(
        ...
        if (finalThreads.size() > 0) {
            PostBox postBoxToWrite = new PostBox(email, Optional.<Long>of(postBox.getNewRepliesCounter().getValue()), finalThreads);
