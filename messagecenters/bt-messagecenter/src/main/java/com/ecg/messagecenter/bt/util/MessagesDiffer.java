...
import com.ecg.messagecenter.bt.cleanup.TextCleaner;
...
public class MessagesDiffer {
    ...
    public TextDiffer.TextCleanerResult diff(DiffInput left, DiffInput right) {
        ...
        DiffInput cleanedLeft = new DiffInput(TextCleaner.cleanupText(left.getText()), left.getConvId(), left.getMessageId());

        DiffInput cleanedRight = new DiffInput(TextCleaner.cleanupText(right.getText()), right.getConvId(), right.getMessageId());
