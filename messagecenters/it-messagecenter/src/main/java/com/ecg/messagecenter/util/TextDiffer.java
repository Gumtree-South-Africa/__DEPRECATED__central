...
public class TextDiffer {
    ...
    static final Pattern QUOTES = Pattern.compile("[>]+[\\s]*");

    private Map<String, List<NGram>> indexedNgramsOfText;

    private List<NGram> ngramsOfText;

    private Map<NGramMatch, NGramMatch> indexedMatches =
                    new TreeMap<NGramMatch, NGramMatch>(new Comparator<NGramMatch>() {
                        @Override public int compare(NGramMatch o1, NGramMatch o2) {
                            return o1.getnGramMatchIndex().compareTo(o2.getnGramMatchIndex());
                        }
                    });

    private Map<NGram, Integer> matchOccurrenceIndex = new LinkedHashMap<NGram, Integer>();
    private Integer succedingMatchesToMatchPhrase;

    private MessagesDiffer.DiffInput diffInput;

    public TextDiffer(MessagesDiffer.DiffInput diffInput) {
        this.diffInput = diffInput;
        this.indexedNgramsOfText = new LinkedHashMap<String, List<NGram>>();
        this.ngramsOfText = initNGrams(diffInput.getText());
    }

    public TextDiffer(MessagesDiffer.DiffInput diffInput, int succedingMatchesToMatchPhrase) {
        this.diffInput = diffInput;
        this.indexedNgramsOfText = new LinkedHashMap<String, List<NGram>>();
        this.ngramsOfText = initNGrams(diffInput.getText());
    ...
    private List<NGram> initNGrams(String originalText) {
        ...
        for (String newlineSection : newlineSplitted) {
            ...
            List<String> emptySpaceSplitted = Lists.newArrayList(
                            Splitter.on(" ").split(newlineSection.trim()).iterator());
    ...
    public TextCleanerResult cleanupMatches() {
        ...
        Map<Integer, NGramMatch> indexedFilteredMatches =
                        filter(new ArrayList<NGramMatch>(indexedMatches.keySet()));
        ...
        LOG.debug("cleanupMatches() for " + diffInput.toString() + " ngrams: " + ngramsOfText.size()
                        + ", cleanup-candidates: " + indexedMatches.size() + ", cleaned-up: " + (
                        ngramsOfText.size() - nGramSizeCounter) + ", Duration: " + (
                ...
                for (int j = 0; j < succedingMatchesToMatchPhrase; j++) {
                    ...
                    if (!indexedFilteredMatches.containsKey(matchToAdd.getnGramMatchIndex())) {
                        if (occurrenceMap2.containsKey(matchToAdd.getnGram())) {
                            occurrenceMap2.put(matchToAdd.getnGram(),
                                            occurrenceMap2.get(matchToAdd.getnGram()) + 1);
                        } else {
    ...
    private void removeFirstNGram(Map<Integer, NGramMatch> filtered,
                    Map<NGram, Integer> occurrenceMap2, NGramMatch match) {
    ...
    public static class NGram {
        ...
        @Override public boolean equals(Object o) {
            ...
            // are changing them)
            if (!textContent.equals(nGram.textContent))
                return false;
    ...
    public static class NGramMatch {
        ...
        @Override public boolean equals(Object o) {
            ...
            if (!nGram.equals(that.nGram))
                return false;
            if (!nGramMatchIndex.equals(that.nGramMatchIndex))
                return false;
    ...
    public static class TextCleanerResult {
        ...
        public static class ProfilingInfo {
            ...
            public ProfilingInfo(long cleanupTimeMillis, long initNgrams, long ngramScanTimeMillis,
                            long ngramApplyTimeMillis) {
