...
public class RemoveDateEmailQuoteHeaderAdvice extends AbstractCleanupAdvice {
    ...
    @Override public void processAdvice() {
        ...
        while (index > 0 && keepChecking && removedLines < 2) {
            ...
            if (text.getAdvice().isLineCleaned(prevIndex) || text.getAdvice()
                            .isLineQuoted(prevIndex)) {
                index--;
                continue; // skip
            }
        ...
        if (removedLines == 0 && index > 1) {
            if (text.getLineWithOriginalIndex(index - 1).content.length() < 10 && lineContainsADate(
                            text.getLineWithOriginalIndex(index - 2).content)
                            && lineContainsAnEmail(
                            text.getLineWithOriginalIndex(index - 2).content)) {
                markQuoted(index - 1);
                markQuoted(index - 2);
