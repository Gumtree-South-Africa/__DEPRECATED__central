...
public class CleanupGumtreeDealerLeadAdvice extends AbstractCleanupAdvice {
    ...
    private static final String DEALER_LEAD_OPENING =
                    "You have received a lead from Gumtree Australia regarding a";
        ...
        for (Text.Line line : text.lines) {
            ...
            if (!text.getAdvice().isLineCleaned(line.originalIndex) && !text.getAdvice()
                ...
                if (line.content.contains(DEALER_LEAD_OPENING)) {
                    ...
                    if (!isDealerLineEnquiryLineDetected
                                    && StringUtils.indexOfAny(line.content, LINES_TO_KEEP) == -1) {
                        markQuoted(line.originalIndex);
                    }
                }
            }
