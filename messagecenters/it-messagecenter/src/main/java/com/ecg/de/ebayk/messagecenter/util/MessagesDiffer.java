...
public class MessagesDiffer {
    ...
    private static final Timer FIND_MATCHES_TIMER =
                    TimingReports.newTimer("message-box.find-matches-timer");
    private static final Timer APPLY_MATCHES_TIMER =
                    TimingReports.newTimer("message-box.apply-matches-timer");

    public String cleanupFirstMessage(String firstMessage) {
        Timer.Context cleanupTimer = CLEANUP_TIMER.time();

        String text = TextCleaner.cleanupText(firstMessage);
    ...
    @Deprecated public TextDiffer.TextCleanerResult diff(DiffInput left, DiffInput right) {

        Timer.Context cleanupTimer = CLEANUP_TIMER.time();

        DiffInput cleanedLeft =
                        new DiffInput(TextCleaner.cleanupText(left.getText()), left.getConvId(),
                                        left.getMessageId());

        DiffInput cleanedRight =
                        new DiffInput(TextCleaner.cleanupText(right.getText()), right.getConvId(),
                                        right.getMessageId());
        ...
        textCleanerResult.setProfilingInfo(new TextDiffer.TextCleanerResult.ProfilingInfo(
    ...
    @Deprecated public static class DiffInput {
        ...
        @Override public boolean equals(Object o) {
            ...
            if (convId != null ? !convId.equals(diffInput.convId) : diffInput.convId != null)
                return false;
            if (messageId != null ?
                            !messageId.equals(diffInput.messageId) :
                            diffInput.messageId != null)
                return false;
