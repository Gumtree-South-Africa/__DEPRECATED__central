...
class PostBoxConflictResolver implements ConflictResolver<PostBox> {
    ...
    private static final Counter POSTBOX_MERGE_COUNTER =
                    TimingReports.newCounter("riak-postbox-merges");
    private static final Histogram POSTBOX_SIBLING_COUNT_HISTOGRAM =
                    TimingReports.newHistogram("riak-postbox-sibling-counts");


    @Override public PostBox resolve(Collection<PostBox> postBoxesToResolve) {
        ...
        return new PostBox(postBoxesToResolve.iterator().next().getEmail(),
                        getHighestCounter(postBoxesToResolve),
                        // we are rather aggressive to show more notification as too less
