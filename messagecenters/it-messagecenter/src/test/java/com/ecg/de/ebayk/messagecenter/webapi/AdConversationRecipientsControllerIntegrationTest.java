 ...
 * at com.ecg.de.ebayk.messagecenter.webapi.AdConversationRecipientsControllerIntegrationTest.testGetBuyerContacts(AdConversationRecipientsControllerIntegrationTest.java:60)
 * <p>
 ...
 */
public class AdConversationRecipientsControllerIntegrationTest extends BaseReplyTsIntegrationTest {

    private static String MESSAGE_CENTER_URI_BASE = "/ebayk-msgcenter";

    @BeforeClass public static void setProperties() {
        System.setProperty("push-mobile.enabled", "false");
    }
     ...
     */
    @Test public void testGetBuyerContacts() throws UnsupportedEncodingException {
        ...
        MailInterceptor.ProcessedMail processedMail1 = testRule.deliver(
                        aNewMail().from("buyer1@buyer.com").to(SELLER_EMAIL).adId(AD_ID)
        ...
        MailInterceptor.ProcessedMail processedMail2 = testRule.deliver(
                        aNewMail().from("buyer2@buyer.com").to(SELLER_EMAIL).adId(AD_ID)
        ...
        testRule.deliver(aNewMail().from("buyerForAnotherAd@buyer.com").to(SELLER_EMAIL).adId("55")
        ...
        String url = "http://localhost:" + testRule.getHttpPort() + MESSAGE_CENTER_URI_BASE + cmd
                        .url();
        System.out.println("url: " + url);

        RestAssured.given().when().get(url).then().statusCode(200).body("body.adId", equalTo(AD_ID))
