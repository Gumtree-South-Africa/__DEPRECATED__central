...
public class PostBoxRepositoryTest {
    ...
    @Test public void persistsPostbox() throws RiakRetryFailedException {
        PostBox box = new PostBox("foo@bar.com", Optional.<Long>absent(),
                        Collections.<ConversationThread>emptyList());
        postBoxRepository.write(box);

        IRiakObject postbox =
                        riakClient.fetchBucket("postbox").execute().fetch("foo@bar.com").execute();
    ...
    @Test public void cleansUpPostbox() throws RiakRetryFailedException {
        PostBox box = new PostBox("foo@bar.com", Optional.<Long>absent(),
                        Collections.<ConversationThread>emptyList());
        postBoxRepository.write(box);
    ...
    @Test public void keepsPostboxEntriesThatAreTooNew() throws RiakRetryFailedException {
        PostBox box = new PostBox("foo@bar.com", Optional.<Long>absent(),
                        Collections.<ConversationThread>emptyList());
        postBoxRepository.write(box);
