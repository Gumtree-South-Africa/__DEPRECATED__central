...
import com.ecg.messagecenter.it.util.TextDiffer;
import org.junit.Test;

import static com.ecg.messagecenter.it.util.TextDiffer.NGram;
import static com.ecg.messagecenter.it.util.TextDiffer.NGramMatch;
...
public class TextDifferTest {
    ...
    @Test public void textCleaner() throws Exception {
    ...
    @Test public void findMatch() throws Exception {
        TextDiffer cleaner = createTextCleaner("das ist   ein\n\n text", 2);

        cleaner.findMatch(new NGram("das", " ", DOESNT_MATTER));

        assertEquals(1, cleaner.getIndexedMatches().size());
        assertEquals(new NGramMatch(new NGram("das", " ", DOESNT_MATTER), 0),
                        cleaner.getIndexedMatches().keySet().iterator().next());
    }

    @Test public void cleanupMatches() throws Exception {
        TextDiffer cleaner = createTextCleaner("das ist   ein\n\n text", 2);
    ...
    @Test public void handleInsertedLinebreaks() throws Exception {
    ...
    @Test public void greedySnipAway1() throws Exception {
    ...
    @Test public void greedySnipAway2() throws Exception {
    ...
    @Test public void resetOnNonFound() throws Exception {
    ...
    @Test public void doNotRemoveOnBoundaryDeletesSection() throws Exception {
    ...
    @Test public void followOrderOfMatches() throws Exception {
        TextDiffer cleaner = createTextCleaner(
                        "das WIRKLICH IGNORE-WEG-DAMIT sind WIRKLICH IGNORE-WEG-DAMIT mehr", 2);

        cleaner.findMatch(new NGram("WIRKLICH", " ", DOESNT_MATTER));
        cleaner.findMatch(new NGram("IGNORE-WEG-DAMIT", " ", DOESNT_MATTER));

        assertEquals("das WIRKLICH IGNORE-WEG-DAMIT sind mehr",
                        cleaner.cleanupMatches().getCleanupResult());
    }


    private TextDiffer createTextCleaner(String text, int succedingMatchesToMatchPhrase) {
        return new TextDiffer(new MessagesDiffer.DiffInput(text, "a:a", "1:1"),
                        succedingMatchesToMatchPhrase);
    }
