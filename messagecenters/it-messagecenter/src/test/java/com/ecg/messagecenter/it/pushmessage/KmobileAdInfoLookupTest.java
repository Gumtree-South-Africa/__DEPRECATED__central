...
public class KmobileAdInfoLookupTest {
    ...
    private static final String AD_WITHOUT_IMAGES =
    ...
    @Test public void adWithImages() throws Exception {
        ...
        assertEquals("http://i.ebayimg.com/00/s/MTYwMFgxMjAw/z/mu8AAOSwBLlVQbzW/$_75.JPG",
                        adInfo.get().getImageUrl());
        assertEquals("Magic mouse", adInfo.get().getTitle());
    }

    @Test public void adWithOneImage() throws Exception {
        httpReturnsPayload(AD_WITH_ONE_IMAGE);

        Optional<AdInfoLookup.AdInfo> adInfo =
                        new AdInfoLookup.AdInfoResponseHandler().handleResponse(httpResponse);

        assertEquals("http://i.ebayimg.com/00/s/MTYwMFgxMjAw/z/ZPEAAOSwcwhVQbsj/$_75.JPG",
                        adInfo.get().getImageUrl());
        assertEquals("Sexy laptop", adInfo.get().getTitle());
    }

    @Test public void adWithoutImages() throws Exception {
    ...
    private void httpReturnsPayload(String adWithImages) throws IOException {
        ...
        when(httpResponse.getEntity().getContent())
                        .thenReturn(new ByteArrayInputStream(adWithImages.getBytes("UTF-8")));
    }
}
