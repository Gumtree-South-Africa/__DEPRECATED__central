...
public class MessagesResponseFactoryTest {
    ...
    @Test
    public void excludeXmlAttachment() {
        attachments.add("foo.png");
        attachments.add("OpenImmoAnfrage.xml");
        addMessage("firstMessage", MessageState.SENT, BUYER_TO_SELLER, "desktop");

        List<MessageResponse> response = createMessagesList();

        assertEquals(1, response.get(0).getAttachments().size());
        assertEquals("/screeningv2/mail/null/INBOUND/parts/foo.png",response.get(0).getAttachments().get(0).getUrl());
    }
    ...
    private void addMessage(String text, MessageDirection messageDirection, String replyChannelInfo) {
        addMessage(text, MessageState.SENT, messageDirection, replyChannelInfo);
    }
