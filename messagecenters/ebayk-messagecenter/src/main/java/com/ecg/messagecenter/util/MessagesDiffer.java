...
public class MessagesDiffer {
    ...
    private MessagePreProcessor messagePreProcessor = new MessagePreProcessor();

    public String cleanupFirstMessage(String firstMessage) {
        return messagePreProcessor.remove(firstMessage);
    }
    ...
    public TextDiffer.TextCleanerResult diff(DiffInput left, DiffInput right) {
        ...
        DiffInput cleanedLeft = new DiffInput(messagePreProcessor.remove(left.getText()), left.getConvId(), left.getMessageId());

        DiffInput cleanedRight = new DiffInput(messagePreProcessor.remove(right.getText()), right.getConvId(), right.getMessageId());
    ...
    public static class DiffInput {
        ...
        public boolean equals(Object o) {
            ...
            if (convId != null ? !convId.equals(diffInput.convId) : diffInput.convId != null) return false;
            if (messageId != null ? !messageId.equals(diffInput.messageId) : diffInput.messageId != null) return false;
            if (!text.equals(diffInput.text)) return false;
