...
public class PostBoxTest {
    ...
    private static final PostBox POST_BOX = new PostBox("" +
            "bla@blah.com",
            Lists.newArrayList(
        ...
        PostBox postBox = new PostBox("" +
                ...
                Lists.newArrayList(
                        new ConversationThread("123", "abc", now(), now().minusHours(100), now(), 1,
                                ...
                                Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(now())),
                        new ConversationThread("321", "cba", now(), now().minusHours(10), now().minusHours(4), 0,
                                Optional.empty(), Optional.empty(), Optional.empty(),
                                Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(now().minusHours(4)))
                )
        );
    ...
    public void marksConversationAsUnread() {
        PostBox postBox = new PostBox("bla@blah.com", Lists.newArrayList(
                new ConversationThread("adid1", "convid2", now().minusDays(4), now(), now(), 0,
