    ...
    private ArrayNode threads(List<ConversationThread> conversationThreads) {
        ...
        for (ConversationThread thread : conversationThreads) {
            ...
            JsonObjects.Builder builder = JsonObjects.builder()
                    ...
                    .attr("numUnreadMessages", thread.getNumUnreadMessages());
            ...
            if (thread.getUserIdSeller().isPresent()) {
                builder.attr("userIdSeller", thread.getUserIdSeller().get());
            }
            if (thread.getLastMessageCreatedAt().isPresent()) {
                builder.attr("lastMessageCreatedAt", nullSafeMillis(thread.getLastMessageCreatedAt().get()));
            }
