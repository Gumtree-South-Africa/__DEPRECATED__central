...
class PostBoxConflictResolver implements ConflictResolver<PostBox> {
    ...
    public PostBoxConflictResolver(int maxConversationAgeDays) {
    ...
    public PostBox resolve(Collection<PostBox> postBoxesToResolve) {
        ...
        return new PostBox.PostBoxBuilder().withEmail(postBoxesToResolve.iterator().next().getEmail()).
                withNewRepliesCounter(getHighestCounter(postBoxesToResolve)).
                withConversationThreads(ImmutableList.copyOf(latest.values())).
                withMaxConversationAgeDays(maxConversationAgeDays).build();

    }

    private Long getHighestCounter(Collection<PostBox> postBoxesToResolve) {
        long counter = 0;
        for (PostBox postBox : postBoxesToResolve) {
