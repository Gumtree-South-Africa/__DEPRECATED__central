...
class JsonToPostBoxConverter {
    ...
    public PostBox toPostBox(String key, String jsonString, int maxConversationAgeDays) {
        ...
        return new PostBox.PostBoxBuilder().withEmail(key).
                withNewRepliesCounter(lookupLongValue(json, "newRepliesCounter")).
                withConversationThreads(threadList).
    ...
    private Long lookupLongValue(JsonNode threadNode, String key) {
        return threadNode.has(key) ? threadNode.get(key).asLong() : 0L;
    }
