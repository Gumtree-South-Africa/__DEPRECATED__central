...
public class PostBoxRepository {
    ...
    private static final Timer COMMIT_TIMER = TimingReports.newTimer("postBoxRepo-commit");
    private static final Timer GET_BY_ID_TIMER = TimingReports.newTimer("postBoxRepo-getById");
    private static final Timer DELETE_TIMER = TimingReports.newTimer("postBoxRepo-delete");
    ...
    PostBoxRepository(IRiakClient riakClient,
        ...
        try {
            String bucketName = bucketPrefix + POST_BOX;
            this.maxConversationAgeDays = maxConversationAgeDays;
            this.postBoxBucket = getPostBoxBucket(riakClient, bucketName, allowSiblings, lastWriteWins);
            this.converter = new PostBoxConverter(bucketName, maxConversationAgeDays);
            this.resolver = new PostBoxConflictResolver(maxConversationAgeDays);
    ...
    private Bucket getPostBoxBucket(IRiakClient riakClient, String bucketName, boolean allowSiblings, boolean lastWriteWins) throws RiakRetryFailedException {
        return riakClient.updateBucket(riakClient.fetchBucket(bucketName).execute())
