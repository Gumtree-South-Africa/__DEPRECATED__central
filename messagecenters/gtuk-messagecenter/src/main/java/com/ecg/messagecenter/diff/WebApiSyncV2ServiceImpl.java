        ...
        if (result.isPresent()) {
            try {
                ...
                String email = conversationThread.getParticipants().stream()
                        ...
                        .findFirst()
                        .orElseThrow(() -> new RuntimeException(String.format("Cannot find user email. UserId: %s, ConversationId: %s", userId, conversationId)));
            ...
            if (conversation.isPresent()) {
                try {
                    ...
                    String email = conversationThread.getParticipants().stream()
                            ...
                            .map(Participant::getEmail)
                            .findFirst()
                            .orElseThrow(() -> new RuntimeException(String.format("Cannot find user email for delete a conversation. UserId: %s", userId)));
