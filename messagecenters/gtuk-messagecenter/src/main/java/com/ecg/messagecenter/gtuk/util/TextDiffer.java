...
public class TextDiffer {
    ...
    private Map<NGramMatch, NGramMatch> indexedMatches = new TreeMap<NGramMatch, NGramMatch>(new Comparator<NGramMatch>() {
        @Override
        public int compare(NGramMatch o1, NGramMatch o2) {
    ...
    public TextCleanerResult cleanupMatches() {
        ...
        Map<Integer, NGramMatch> indexedFilteredMatches = filter(new ArrayList<NGramMatch>(indexedMatches.keySet()));
    ...
    private boolean escapedWhitespace(NGram nGram) {
        return nGram.getTextContent().contains("\\t") || nGram.getTextContent().contains("\\n");
    }
