...
public class MessageIdHeaderEncryption {
    ...
    private static final byte[] KEY_BYTES = new byte[]{
            56, -72, 54, 122, 32, -29, 75, -109, 101, -127, -102, -84, 101, -127, -102, -84};
    private static final Charset US_ASCII = Charset.forName("US-ASCII");

    private static final SecretKeySpec KEY_SPEC;

    static {
        Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider()); // NOSONAR - PMD false positive bug in latest sonar ("Avoid Thread Group")
    ...
    public String encrypt(String messageId) {
        if (!messageId.trim().equals(messageId)) {
            throw new IllegalArgumentException("Trailing/leading spaces in message id are not allowed");
        }
