...
public class Text {
    ...
    private String basicCleanup(String line) {
        ...
        final String all3 = trimmed.replaceAll("[\\s]+", " ");
        final String all2 = all3.replaceAll("\\s[^a-zA-Z0-9&à:]+\\s", " ");
        final String all1 = all2.replaceAll("\\s[-_*]+\\s", "");
        final String all = all1.replaceAll("^[-_*]+$", "");
        return all;
    }
