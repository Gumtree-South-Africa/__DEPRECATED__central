...
class PostBoxConverter implements Converter<PostBox> {
    ...
    public IRiakObject fromDomain(PostBox postBox, VClock vClock) throws ConversionException {
        ...
        return RiakObjectBuilder.newBuilder(RiakPostBoxRepository.POST_BOX, postBox.getUserId())
                .withValue(compressed)
                .addIndex(RiakPostBoxRepository.UPDATED_INDEX, DateTime.now().getMillis())
                .withVClock(vClock)
