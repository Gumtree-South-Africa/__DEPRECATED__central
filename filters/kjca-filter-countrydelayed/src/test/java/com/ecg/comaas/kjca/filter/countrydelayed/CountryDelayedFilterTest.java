...
public class CountryDelayedFilterTest {
    ...
    private CountryDelayedFilter objectUnderTest;
    ...
    public void whenTnsApiReturnsFalse_shouldReturnEmptyFeedback() {
        when(mailMock.getUniqueHeader(SENDER_IP_ADDRESS.getHeaderName())).thenReturn(IP_ADDRESS);
        when(tnsApiClientMock.getJsonAsMap(anyString())).thenReturn(Collections.singletonMap(CountryDelayedFilter.IS_COUNTRY_DELAYED_KEY, Boolean.FALSE));
    ...
    public void whenTnsApiReturnsTrue_shouldReturnFeedback() {
        when(mailMock.getUniqueHeader(SENDER_IP_ADDRESS.getHeaderName())).thenReturn(IP_ADDRESS);
        when(tnsApiClientMock.getJsonAsMap(anyString())).thenReturn(Collections.singletonMap(CountryDelayedFilter.IS_COUNTRY_DELAYED_KEY, Boolean.TRUE));

        List<FilterFeedback> actualFeedback = objectUnderTest.filter(messageContextMock);

        assertThat(actualFeedback).containsExactly(
                new FilterFeedback("country is delayed", "IP country is delayed", SCORE, FilterResultState.HELD)
        );
    }
}
