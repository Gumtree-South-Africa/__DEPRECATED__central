...
@ContextConfiguration(classes = GumtreeGeoIpFilterBannedTest.TestContext.class)
public class GumtreeGeoIpFilterBannedTest {
    ...
    @Test
    public void geoIPIsNotAppliedInS2BDirection() {
        MessageProcessingContext messageProcessingContext = getMessageProcessingContext("NG", BANNED_COUNTRY_IP, MessageDirection.SELLER_TO_BUYER);
    ...
    public void testUnknownCountry() {
        MessageProcessingContext messageProcessingContext = getMessageProcessingContext(null, UNKNOWN_IP);
    ...
    @Test
    public void testKnownButUnconfCountry() {
    ...
    public void testDoCheckForBannedCountry() {
        ...
        assertEquals(feedbackList.get(0).getResultState(), FilterResultState.DROPPED);
        assertEquals(feedbackList.get(0).getUiHint(), BANNED_COUNTRY_IP);
    }

    private MessageProcessingContext getMessageProcessingContext(String country, String ip) {
        return getMessageProcessingContext(country, ip, MessageDirection.BUYER_TO_SELLER);
    }

    private MessageProcessingContext getMessageProcessingContext(String country, String ip, MessageDirection direction) {
        Message message = mockMessage(direction);
    ...
    static class TestContext {
        ...
        public GeoIpFilterConfig filterConfig() throws Exception {
            return new GeoIpFilterConfig.Builder(State.ENABLED, 1, Result.DROP)
                    .withExemptedCategories(exemptedCategories)
                    .withCountrySet(ImmutableList.of("NG"))
                    .build();
        }
